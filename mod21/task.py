from sqlalchemy import Table, MetaData, create_engine, Column, Integer, String, Date, DateTime, Float, Boolean, ForeignKey, func
...
@app.route('/books_author/<int:author_id>')
def books_author(author_id):
...
@app.route('/books_student/<int:student_id>')
def books_student(student_id):
    if request.method == 'GET':
        books_id = session.query(Receiving_book.book_id).filter(Receiving_book.book_id == Book.id, Receiving_book.student_id == student_id).all()
        books_id = [author[0] for author in books_id]
        need_books = session.query(Book.author_id).filter(Book.id.in_(books_id)).all()
        need_books = [book[0] for book in need_books]
        books_by_authors = session.query(Book).filter(Book.author_id.in_(need_books)).filter(Book.id.notin_(books_id)).all()
        books_list = []
...
def book_of_the_month():
    ...
    return f'Среднее количество книг, взятых студентами в этом месяце, составляет {avg_book_month_count}', 200


@app.route('/books/popular', methods=['GET'])
def the_most_popular_book():
    book_id = session.query(func.count(Receiving_book.book_id)).filter(Receiving_book.student_id == Student.id, Student.average_score >= 4.0).group_by(Receiving_book.book_id).order_by(func.count(Receiving_book.book_id).desc()).limit(1).all()
    book = session.query(Book).filter(Book.id == book_id[0][0]).all()[0]
    json_book = book.to_json()
...
def students():
    if request.method == 'GET':
        ...
        students = session.query(Student). \
            filter(Receiving_book.student_id == Student.id, Receiving_book.date_of_issue >= current_year_start).group_by(Receiving_book.student_id).order_by(func.count(Receiving_book.book_id).desc()).limit(10).all()
        students_list = []
        ...
        if stud_file:
            try:
                stud_file.save('students_file.csv')
                stud_list = []
                with open('students_file.csv', 'r', newline='') as csvfile:
